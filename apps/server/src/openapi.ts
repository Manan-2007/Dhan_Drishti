...
export const openApiSpec = {
  ...
  components: {
    ...
    schemas: {
      ...
      Transaction: { type: "object", properties: { id: { type: "string" }, portfolioId: { type: "string" }, accountId: { type: ["string", "null"] }, securityId: { type: ["string", "null"] }, type: ref("TxType"), tradeDate: { type: "string" }, settleDate: { type: ["string", "null"] }, quantity: ref("Money"), price: ref("Money"), grossAmount: ref("Money"), fees: ref("Money"), taxes: ref("Money"), currency: { type: "string" }, fxRateToBase: { type: ["string", "null"] }, segment: { type: "string" }, sourceBroker: { type: ["string", "null"] }, notes: { type: ["string", "null"] }, security: { type: ["object", "null"], description: "The security this row refers to (joined in for display — never null on a buy/sell)", properties: { id: { type: "string" }, symbol: { type: "string" }, name: { type: "string" }, isin: { type: ["string", "null"] }, assetClass: { type: "string" }, sector: { type: ["string", "null"] }, exchange: { type: ["string", "null"] } } }, account: { type: ["object", "null"], description: "The broker account this row was recorded against, if any", properties: { id: { type: "string" }, name: { type: "string" }, broker: { type: "string" } } } } },
      ...
      ImportInput: { type: "object", required: ["portfolioId", "broker", "filename", "content"], properties: { portfolioId: { type: "string" }, accountId: { type: ["string", "null"] }, broker: { type: "string" }, filename: { type: "string" }, content: { type: "string", description: "Raw CSV text, or base64 when encoding is set (an .xlsx workbook, a CAS PDF, or non-UTF8 CSV)" }, encoding: { type: "string", enum: ["base64"], description: "Set when content is base64 (an .xlsx or CAS PDF upload)" }, casPassword: { type: "string", description: "Password for a CAS PDF import (broker \"cas\") — usually the PAN" }, from: { type: "string", description: "Start of the period this file covers (with replace)" }, to: { type: "string" }, replace: { type: "boolean", description: "Overwrite existing rows from this source within [from, to]" }, mapping: { type: "object", description: "Column mapping (required for the generic adapter)" } } },
